			...
			for (var key in ast) {
				...
				if (key === "type") {
					if (value === "FunctionDeclaration") {
						name = ast.id.name + toParamString(ast.params);
						newContainer = {
							label: name,
							children: null,
							line: toLine(ast.range[0], root.linebreaks),
							text: "xyz"
						};
					...
					if (value && value.type && value.type === "FunctionExpression") {
						...
						if (ast.key && ast.key.name) {
							...
							newContainer = {
								label: ast.key.name + toParamString(value.params),
								children: null,
								line: toLine(value.range[0], root.linebreaks),
								text: ""
							};
							_addChild(container, newContainer);
							container = newContainer;
